...
check_for_sources()
{
	...
	if [ ! -d ${CROSS_COMPILER} ]; then
		if [ ! -f ${CROSS_COMPILER_ARCHIVE} ]; then
			echo "Cross compiler ${CROSS_COMPILER_ARCHIVE} not found,downloading ${CROSS_COMPILER_ARCHIVE}"
			wget http://${CROSS_COMPILER_SERVER}/${CROSS_COMPILER_ARCHIVE}
			if [ $? != "0" ]; then
				echo "${CROSS_COMPILER_ARCHIVE} not found on server ${CROSS_COMPILER_SERVER}"
				exit
			fi
		fi
		echo -n "Extracting cross compiler ${CROSS_COMPILER_ARCHIVE}... "
